...
//                }
    ...
    void resetearBoletin(ActionEvent event) {
        ...
        if (mt != null) {
            queryReset = "UPDATE datagest.descarga SET estadoCruce=0 where idDescarga=" + mt.getId();
            queryClean = "DELETE from datagest.crucetestra where codigoEdicto=" + Varios.entrecomillar(mt.getCodigo());
            try {
...
//                Logger.getLogger(WinC.class.getName()).log(Level.SEVERE, null, ex);
    ...
    void resetearDia(ActionEvent event) {
        ...
        if (fecha != null) {
            queryReset = "UPDATE datagest.descarga SET estadoCruce=0 where fecha=" + Varios.entrecomillar(Dates.imprimeFecha(fecha));
            queryClean = "DELETE from datagest.crucetestra where fechaPublicacion=" + Varios.entrecomillar(Dates.imprimeFecha(fecha));
            try {
                Sql bd = new Sql(Var.con);
                bd.ejecutar(queryReset);
                bd.ejecutar(queryClean);
